...
| `slicer`          | `in: analog`                 | `out: digital` | `threshold: float?`、`hysteresis: float=0.2×幅值`、`names: list[str]?`                                                          | Schmitt 切片（**向量化滞回**：仅对越过上/下阈值的定态样本做相邻差分，死区样本继承前态；O(n) 时间、O(定态数) 内存）；threshold 缺省 = (Vmin+Vmax)/2；每路模拟→1 个数字通道（名字继承或由 names 指定）；阈值回写 `meta.threshold_v`        |      |
...
| `uplink_precond`  | `in: analog`                 | `out: analog`  | `channel: str`、`profile: str`、`chip_s: float?`                                                                              | 上行预条件：抽取到 ~12 样点/chip + 1ms 滑动均值 HPF 剥离 60Hz 包络（ADR-010）                                          |      |
