...
| `slicer`          | `in: analog`                 | `out: digital` | `threshold: float?`、`hysteresis: float=0.2×幅值`、`names: list[str]?`                                                          | Schmitt 切片；threshold 缺省 = (Vmin+Vmax)/2；每路模拟→1 个数字通道（名字继承或由 names 指定）；阈值回写 `meta.threshold_v`        |      |
